...
std::stack<Pair> Pathfinding::aStarSearch(int grid[][COL], Pair src, Pair dest)
{
    std::stack<Pair> final_path;
    ...
    while (!openList.empty()) {
        ...
        /*
         ...
         E -->  East        (i, j+1)
         W -->  West           (i, j-1)
         N.E--> North-East  (i-1, j+1) offline
         N.W--> North-West  (i-1, j-1) offline
         S.E--> South-East  (i+1, j+1) offline
         S.W--> South-West  (i+1, j-1) offline*/
...
void Pathfinding::update_grid(int _grid[ROW][COL])
{
    //map->LoadMap(_grid);
    ...
    std::cout << "////////////////////finish grid///////////////////" << std::endl;
}

void Pathfinding::walk(std::stack<Pair> path, int soldier)
{
    ...
    while (!path.empty()) {
        ...
        for (int row = 0; row < ROW; row++) {
            for (int column = 0; column < COL; column++) {
                if (row == p.first && column == p.second) {
                    refresh_grid[row][column] = soldier;

                } else if (main_grid[row][column] == soldier) {
                    refresh_grid[row][column] = 0;
                } else {
                    refresh_grid[row][column] = main_grid[row][column];
                }
            }
...
Pathfinding::Pathfinding() {
    ...
    soldeir4 = std::make_pair(8, 0);
    soldeir5 = std::make_pair(8, 0);
    soldeir6 = std::make_pair(8, 0);
    soldeir7 = std::make_pair(8, 0);
    soldeir8 = std::make_pair(8, 0);
    soldeir9 = std::make_pair(8, 0);
    soldeir10 = std::make_pair(8, 0);
    soldeir11 = std::make_pair(8, 0);
    soldeir12 = std::make_pair(8, 0);
    soldeir13 = std::make_pair(8, 0);
    soldeir14 = std::make_pair(8, 0);
